...
let phrase = ''
const getMessage = (heroes) => {
 heroes.forEach(({name,realName,gender}, index) => {

 const sex = gender ==='Male'? 'do heroí':'da heroína'


 phrase += `O nome ${sex} no ${`${index + 1}º`} item é ${name} 
 e seu nome verdadeiro é ${realName}. \n\n`
 })
 return phrase
}
//console.log(getMessage(getHeroes()))
  ...
  */
  ...
  const getScore = userResponses => {
    const lastReply = userResponses[3]
  ...
  */
  ...
  const getScore = userResponses => {
    const lastReply = userResponses[userResponses.length - 1]
