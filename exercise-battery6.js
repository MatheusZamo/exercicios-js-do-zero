...
const pegaNomeConcurso = (id) => {
    let nomeConcurso = ''
    concursos.forEach(concurso => concurso.id === id ? 
        nomeConcurso = concurso.nome : 'Concurso não encontrado')

        return nomeConcurso
}

//console.log(pegaNomeConcurso('d8sduqsd3'))
...
const media = (lista) =>{
    let valor = 0
    lista.forEach(lista => valor += lista.custoTotal)
    return valor / 5
}
//console.log(`A média é ${media(custos)}`)
/*
05
...
const palavrasMaior3 = (lista) => {
  let palavrasRetornadas = []
  lista.forEach((palavra) => {
    if(palavra.length > 3){
      palavrasRetornadas.push(palavra)
    }
  })
  return palavrasRetornadas
}
//console.log(palavrasMaior3(palavras))
/*
06
